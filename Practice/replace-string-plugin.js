...
function replaceContent(result, [patterns = [], replacement = []]) {
    if (Array.isArray(patterns) && Array.isArray(replacement) && patterns.length === replacement.length) {
...
function copyFile({ file, env = "", sourcePath, distPath, replaceExtname = [] }) {
    if (["html", "htm", "css", "js", "jsx", "json", "md", ...replaceExtname].includes(extname(file).replace(".", ""))) {
        return true;
    }
    fs.writeFileSync(join(distPath, env, file), fs.readFileSync(join(sourcePath, file), { encoding: null }));
...
ReplaceStringPatternPlugin.prototype.apply = function(compiler) {
    ...
    const distPath = config.distPath || `${compiler.options.output.path}/multiple-bundle-from-string-replace`;

    const eventDone = (compilation, done) => {
        console.log(`[webpack-plugin-create-multiple-bundle-from-string-replace]: In ${distPath}`);
