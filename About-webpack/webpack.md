...
- `webpack -d` - to include sourse maps
- `webpack --colors` - for making things beauty

在package.json中写入script

```javascript
{
    scripts:{
        dev:'webpack-dev-server --devtoold eval --progress --colors',
        pro:'NODE_ENV=production webpac -p'
    }
}
```
...
module.exports = {
  // entry: Obj/Arr/Str
  // 当entry是一个对象的时候，output中的[name] 就是指向的entry中的健。
  entry: {
    bundle1: './main1.js',
    bundle2: './main2.js'
  },
